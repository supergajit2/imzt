...
module.exports = {
    ...
    checkIncomplete: async function(page, selector) {
        ...
        if (totalIncomplete > 0 ) {        
            ...
            if ( totalIncomplete > youngerIncomplete ) {
                await helpers.click(page,report.YOUNGER)
                const seniorCount = await helpers.getCount(page, selector)
                expect(seniorIncomplete).toBe(seniorCount)
                console.log(`Senior records: ${seniorIncomplete}`)
            } else {
                console.log(`Junior incomplete: ${youngerIncomplete}`)
            }
